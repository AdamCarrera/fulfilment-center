...
def get_fulfillmen_location_status(id):
    res = requests.get(f"http://192.168.2.27:9080/fulfillment-locations/{id}", 
                        headers={"Content-Type":"application/json"}) 
    return res.json()

# Sends a command for a specific robot for speed and destination
def post_robot_command(id, destination, speed):
    res = requests.post(f"http://192.168.2.27:9080/devices/{id}/params", 
                        headers={"Content-Type":"application/json"}, 
                        data=json.dumps({
...
def get_robot_status(id):
    res = requests.get(f"http://192.168.2.27:9080/devices/{id}", 
                        headers={"Content-Type":"application/json"}, 
                    )
    return res.json()
